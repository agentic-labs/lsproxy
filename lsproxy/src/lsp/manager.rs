...
impl Manager {
    ...
    pub async fn prepare_call_hierarchy(
        ...
        let workspace_files = self.list_files().await.map_err(|e| {
            error!("[PrepareCallHierarchy] Failed to retrieve workspace files: {}", e);
            LspManagerError::InternalError(format!("Workspace file retrieval failed: {}", e))
        })?;
        ...
        match &result {
            ...
            Err(e) => {
                error!("[PrepareCallHierarchy] Failed to prepare call hierarchy: {}", e);
            }
        }
    ...
    pub async fn outgoing_calls(
        ...
        let pkg = locked_client.get_narrowest_package(item.uri.path()).await.map_err(|e| {
        ...
        let obj = locked_client.get_referenced_object(&pkg, item.uri.path(), item.selection_range.start).await
            .map_err(|e| {
        ...
        if let Some(obj) = obj {
            ...
            debug!("[OutgoingCalls] Found {} potential function calls", call_ranges.len());
            ...
            for range in call_ranges {
                    ...
                    .await
                {
                    debug!(
                        ...
                        target_obj.name,
                        target_obj.file_path
                    );
                    let target_item = CallHierarchyItem {
                        name: target_obj.name,
                        kind: SymbolKind::FUNCTION,
                        tags: None,
                        detail: Some(format!("{} • {}", PathBuf::from(&target_obj.file_path).parent().and_then(|p| p.file_name()).and_then(|s| s.to_str()).unwrap_or(""), 
